...
const CAMPOS_XML = {
  ...
  // Valor apurado do ISSQN (o "ISS normal") — separado do ISS retido, que
  // depende também da flag tpRetISSQN (ver extrairDadosNota).
  issqn: ["vISSQN", "ValorIssqn"],
  // "Leiaute prevê 3 opções" pra tpRetISSQN, segundo a Nota Técnica nº 008
  // — ainda não confirmamos os 3 códigos exatos contra o manual oficial,
  // só que "2" aparenta ser "retido pelo tomador" (convenção mais comum
  // nesse tipo de campo). Confirmar assim que aparecer uma nota com ISS
  // retido de verdade.
...
function extrairDadosNota(xml, cnpjCliente) {
  ...
  return {
    ...
    issqn,
    // Só reporta como "retido" quando a flag da nota diz que foi retido
    // pelo tomador — do contrário fica em branco (não confundir com o
    // ISSQN apurado, que é sempre preenchido quando existe).
