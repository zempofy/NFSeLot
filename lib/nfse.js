...
export async function buscarNotas({ pfxBuffer, senha, cnpjCliente, dataInicial, dataFinal, tipo, formato }) {
  ...
  for (let i = 0; i < LIMITE_DE_SEGURANCA; i++) {
    const { body } = await requisitarComCertificado({ chavePem, certPem, path: PATHS.dfe(nsuAtual) });
    ...
    for (const item of lote) {
      ...
      if (xmlBase64) {
        ...
        try {
          const r = await requisitarComCertificado({ chavePem, certPem, path: PATHS.nfse(chaveAcesso) });
